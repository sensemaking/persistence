...
namespace Sensemaking.Dapper.Bdd;
...
public abstract class TruncateDbSpecification : Specification
{
    ...
    private void Truncate(string connectionString, string[] excludeFromTruncate)
    {
        ...
        new Db(connectionString).QueryAsync<string>(
                """
            ...
            .ForEach(tt => new Db(connectionString).ExecuteAsync($"DELETE FROM {tt}", commandType: CommandType.Text).Await());
    }
}
