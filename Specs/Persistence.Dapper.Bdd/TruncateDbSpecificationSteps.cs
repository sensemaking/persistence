...
public partial class TruncateDbSpecificationSpecs
{
    ...
    protected override void before_all()
    {
        ...
        using (var con = new SqlConnection(Startup.Database.connection_string))
        {
            con.Execute($"""
                         ...
                         CREATE TABLE {Startup.Database.static_data_table_name} (Id UNIQUEIDENTIFIER NOT NULL);
                         INSERT INTO {Startup.Database.static_data_table_name} SELECT @id;
    ...
    protected override void after_all()
    {
        using (var con = new SqlConnection(Startup.Database.connection_string))
            con.Execute($"""
                         ...
                         DROP TABLE {Startup.Database.static_data_table_name};
    ...
    private void excluded_table_data_is_not_removed()
    {
        ...
        new Db(Startup.Database.connection_string).Query<dynamic>($"SELECT * FROM {Startup.Database.static_data_table_name}", commandType: CommandType.Text).ToArray().Length.should_be(1);
        new Db(Startup.Database.connection_string).Query<dynamic>($"SELECT * FROM {table_name_to_skip}", commandType: CommandType.Text).ToArray().Length.should_be(1);
    }
